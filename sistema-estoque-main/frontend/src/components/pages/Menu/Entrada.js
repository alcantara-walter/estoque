...
function Entrada() {
    ...
    async function AdicionarEntrada(produtos) {
        let msgType = 'sucesso';
        let data;
    ...
    formData.append('produtos', produtosFormData)
        ...
        try {
            data = await api.patch(`/produtos/acrescentarQuantidade`, formData, {
                headers: {
                    Authorization: `Bearer ${JSON.parse(token)}`,
                    'Content-Type': 'application/json',
                  },
                  body: JSON.stringify(AdicionarEntrada),
                }).then((response) => {
